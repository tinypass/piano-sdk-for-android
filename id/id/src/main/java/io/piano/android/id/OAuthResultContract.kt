...
internal class OAuthResultContract : ActivityResultContract<String, OAuthResult?>() {
    ...
    override fun createIntent(context: Context, input: String): Intent = client.buildSocialAuthIntent(context, input)

    override fun parseResult(resultCode: Int, intent: Intent?): OAuthResult? = runCatching {
        when (resultCode) {
            Activity.RESULT_OK -> {
